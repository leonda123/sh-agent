...
class DocHistoryTasks:
    ...
    def analyze_content_task(self, agent, process_task):
        return Task(
            ...
            3. [信息提取]：
               ...
               - 提取文档首页或封面的当前版本号信息。**重要定义：首页指的是 Markdown 文件的最开头部分（通常在第一个一级标题或目录之前）。你绝不能把文档其他页面（如各章节页眉处提取的信息 `| A92024001-CBB-CUPSAC | B |` 等）当做首页信息。**如果在这部分（最开头的内容）找不到明确的版本号信息，请如实记录为“未找到”，**严禁自行捏造或去正文页眉提取**。必须截取包含该版本号的首页原文片段作为出处证明。
               - 提取文档首页或封面的更改单据/单据号信息，同样遵循上述首页的定义截取原文片段。如果找不到，如实记录。
            4. [数据输出]：生成一份结构化提取清单，包含：
               - 提取到的所有版本历史记录明细（必须保留原文证据）
               - 首页提及的版本号与单据号（**必须包含首页真实的原文出处，严禁拿页眉或表格内容凑数；若首页无此信息，必须标明“首页无版本信息”**）
    ...
    def analyze_content_task_b(self, agent, process_task):
        return Task(
            ...
            3. [独立提取]：重新提取所有历史版本条目及对应字段，以及首页的版本号和单据号。**首页定义同A轮：指 Markdown 文件最开头（一级标题或目录前）。绝不能用文档内部的页眉（如 `| A92024001-CBB-CUPSAC | B |`）当做首页信息。必须包含首页原文片段作为出处证明；如在首页区域未找到，严禁捏造或去正文找，直接记录“未找到”。**
            4. [数据输出]：生成你的独立结构化清单。
    ...
    def review_and_report_task(self, agent, verify_task):
        return Task(
            ...
            3. 最后一行的版本（即最新版本）应该为当前版本，并检查**首页区域**（见前期定义，仅限文档最前部的非章节、非页眉区域）是否提及该版本。**重要判定标准：如果只能从文档内部的页眉（如 `| A92024001-CBB-CUPSAC | B |` 等）、正文页或目录中找到版本信息，而在真正的首页封面区域没有找到，此项必须判为“不符合”，并标注“首页未找到版本信息（注：正文页眉信息无效）”。严禁将正文页眉当作首页证据。如果找到了真实的首页版本号，必须在备注中列出真实的首页原文片段作为出处证明。**
